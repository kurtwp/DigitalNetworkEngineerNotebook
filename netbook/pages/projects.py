...
from netbook.database import get_all_projects, create_project, delete_project, update_project
from netbook.theme import (
    apply_global_styles, DARK_BG, PANEL_BG, BORDER, ACCENT, ACCENT_DIM,
    TEXT_PRI, TEXT_SEC, TEXT_MUTED, STATUS_COLORS
)
...
def projects_page():
    ...
    with ui.element("div").style(
            ...
            with cards_col:
                if not projects:
                    with ui.element("div").style(
                        ...
                        ui.icon("folder_open").style(f"font-size:44px; color:{TEXT_MUTED};")
        ...
        with ui.dialog() as new_dialog, ui.card().style(
            ...
            ticket_in = ui.input("Ticket Number (optional)").style("width:100%; margin-top:12px;")
            type_in   = ui.select(
                ["", "Decommission", "New Install", "Migration", "Upgrade", "Troubleshoot", "Lab", "Other"],
                label="Type of Work", value=""
            ).props('outlined').style("width:100%; margin-top:12px;")
            date_in   = ui.input("Scheduled Date (MM-DD-YYYY)").style("width:100%; margin-top:12px;")
            status_in = ui.select(
        ...
        new_btn.on("click", new_dialog.open)


def _project_card(project, refresh_cb):
    ...
    with ui.element("div").style(
        ...
        # Status dot — fixed width, no shrink
        ui.element("span").style(
            f"width:9px; height:9px; border-radius:50%; background:{sc};"
            f"flex-shrink:0; display:inline-block;"
        )

        # Text block — takes all available space, overflow hidden
        with ui.column().style("gap:2px; flex:1; min-width:0; overflow:hidden;"):
            # Row 1: ticket number (prominent) + name
            with ui.row().style("align-items:baseline; gap:10px; flex-wrap:nowrap; min-width:0;"):
                if ticket:
        ...
        with ui.row().style("gap:8px; align-items:center; flex-shrink:0;"):
            ...
            open_btn.on("click", lambda pid=project["id"]: ui.navigate.to(f"/worknotes/{pid}"))
            ...
            def confirm_delete(pid=project["id"], pname=project["name"]):
